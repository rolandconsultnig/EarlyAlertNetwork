// export-mysql-schema.js
// Script to export MySQL database schema only (no data) for cPanel deployment
...
// Function to export the database schema only
async function exportDatabaseSchema() {
  console.log('=== MySQL Database Schema Export ===');
  ...
  try {
    ...
    const outputFile = path.join(outputDir, `${config.database}_schema_${timestamp}.sql`);
    
    // Start writing SQL file
    let sqlContent = `-- IPCR Early Warning & Response System - Database Schema Export\n`;
    sqlContent += `-- Generated: ${new Date().toISOString()}\n`;
    sqlContent += `-- Database: ${config.database}\n\n`;
    sqlContent += `-- This file contains only the database structure (no data)\n\n`;
    ...
    // Add creation of admin user comment
    sqlContent += `\n-- IMPORTANT: Don't forget to add an admin user after deploying\n`;
    sqlContent += `-- Example admin user creation:\n`;
    sqlContent += `-- INSERT INTO users (username, password, email, fullName, role, securityClearance)\n`;
    sqlContent += `-- VALUES ('admin', 'hashed_password', 'admin@example.com', 'System Administrator', 'admin', 7);\n`;
    
    // Write the SQL file
    fs.writeFileSync(outputFile, sqlContent);
    
    console.log(`Database schema exported successfully to: ${outputFile}`);
...
// Run the database schema export
